...
int busscan(char *linebuf) {
	...
	if ((pn = addnode()) == NULL) {
		msg(stderr, "busscan: can't add new node\n");
		return 0;
	}
	...
	name[12] = '\0';
	makenode(pn);
	msg(stderr, "busscan: scaned bus %d\n", pn->no);
	return 1;
}
...
int branchscan(char *linebuf) {
	...
	if ((pb = addbranch()) == NULL) {
		msg(stderr, "branchscan: can't input new branch\n");
		return 0;
	}
	...
	&minvolt, &maxvolt);
	if (n != 21) {
		msg(stderr, "branchscan: at cdf line %d:\n%s\n", lineno, linebuf);
		return 0;
	}
	msg(stderr, "branchscan: scaned branch %d-%d\n", tbusno, zbusno);
...
struct comp branchcalc(struct branch *b) {
	...
	case FT: case VT:	/* fixed & variable tap transformers */
		...
		break;
	default:
		msg(stderr, "cdf error: wrong branch type, %d\n", b->type);
...
void nodebranch(struct node *pn, struct branch *pb) {
	...
	/* yik0 */
   	if (pn == pb->inode) {
	    	...
	    	msg(stderr, "nodebrnch: node doesn't relate to branch\n");
		return;
	}
...
int initdata(FILE *fp) {
	if (readcdf(line, MAXLINE, fp) != TITLE) {
		msg(stderr, "cdf error: missing title card\n");
		return 0;
	}
	titlescan(line);
	if (readcdf(line, MAXLINE, fp) != BUS) {
		msg(stderr, "cdf error: missing bus card\n");
		return 0;
	}
	...
	if (readcdf(line, MAXLINE, fp) != BRANCH) {
		msg(stderr, "cdf error: missing branch card\n");
		return 0;
	}
	while (readcdf(line, MAXLINE, fp) != END)
		if (!branchscan(line))
